...
/* standard output */
	...
	if (bopen(ofp,BIO_STDOUT,"wct",0666) < 0) return BAD ;
...
#if	CF_DEBUG
	if (g.debuglevel > 0)
	    eprintf("main: f_exitargs %d - ai %d\n",f_exitargs,ai) ;
#endif
...
#if	CF_DEBUG
	if (g.debuglevel > 0)
	    eprintf("main: about to check arguments\n",
	        progname) ;
#endif
...
/* check out the various addresses that we may be using */
	...
	if (g.debuglevel > 0) {
	    ...
	    eprintf("main: done with address to\n") ;
...
/* continue address preparation */
	...
	if (g.debuglevel > 0) {
	    ...
	    eprintf("main: about to create a job file\n") ;

	    eprintf("main: w/ tmpdir > %s\n",tmpdir) ;
...
#if	CF_DEBUG
	if (g.debuglevel > 0)
	    eprintf("main: created job file > %s\n",jobfname) ;
#endif
...
#if	CF_DEBUG
	if (g.debuglevel > 0) eprintf("main: 7\n") ;
#endif
...
#if	CF_DEBUG
	if (g.debuglevel > 0) eprintf("main: 8\n") ;
#endif
...
#if	CF_DEBUG
	    if (g.debuglevel > 0)
	        eprintf("main: go through the loops\n") ;
#endif
...
#if	CF_DEBUG
	    if (g.debuglevel > 0)
	        eprintf("main: about to close input file\n") ;
#endif
...
#if	CF_DEBUG
	if (g.debuglevel > 0)
	    eprintf("main: about to close job file\n") ;
#endif
...
#if	CF_DEBUG
	if (g.debuglevel > 0)
	    eprintf("main: about to use transport if real job\n") ;
#endif
...
char	queue_machine[] ;
{
	...
	if ((rs = bopencmd(fpa,bbuf)) >= 0) {
	    ...
	    pid = rs ;
	    rs = BAD ;
	    while ((l = breadln(pfp,bbuf,blen)) > 0) {
...
#if	CF_DEBUGS
	eprintf("avail_uucp: past the command OK\n") ;
#endif
