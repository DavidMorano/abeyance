...
/*******************************************************************************
	...
	Standard Automation Environment Variables :
		...
		A_SATCONFIG	SATtool program configuration file
		A_AUTOCONFIG	AutoRun program configuration file
		A_LIBDIR	library directory
		A_RELEASE	SW release
...
#define	KEY_LIB		1	/* library directory */
#define	KEY_LOG		2	/* log file */
#define	KEY_SYS		3	/* system record */
#define	KEY_FUN		4	/* functional command */
#define	KEY_DEF		5	/* default system name */
#define	KEY_REL		6	/* default SUT release */
#define	KEY_FILT	7	/* SAT filter file */
#define	KEY_MAP		8	/* SAT mapping file */
#define	KEY_DICT	9	/* SAT dictionary file */
#define	KEY_EXP		10	/* environment variable for export */
#define	KEY_WAIT	11	/* lock file wait time out */
#define	KEY_OVERLAST	12	/* number of key types */

struct namelist		cfk[] = { 	/* configuration file key */
	"control", 1, 7,
	"library", 1, 3,
	"logfile", 2, 3,
	"system", 1, 1,
...
int main(int argc,cchar **argv,cchar **envv)
{
	...
	if (bopen(ofp,BOUT,"wc",0666) < 0) return BAD ;
...
/* create a temporary pipe file */
	...
	if (i >= TMPTRIES) {
	    ...
	    bprintf(efp,
	        "%s: could not create the temporary pipe file\n") ;
	        progname) ;
...
/* capture the lock if specified */
	        ...
	        if (funopt & FO_LOCK) {
	            ...
	            while (k > 0) {
	                ...
	                rs = uc_open(lockfname,O_RDWR | O_CREAT | O_EXCL,0664) ;
...
baddir:
	bprintf(efp,"%s: bad working directory specified (%d)\n",
	    progname,rs) ;
