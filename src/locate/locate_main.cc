...
#ifndef	LOCATE_DB
#define	LOCATE_DB	"/usr/add-on/gnu/var/locate/locatedb"
#endif
...
local char * last_literal_end (char *name) noex {
...
local int locate(char *pathpart,char * dbfile) noex {
...
/* To check the age of the database.  */
	...
	pathsize = MAX(MAXPATHLEN,1026) ; /* Increased as necessary by getstr */
...
/****
	If the previous path matched, scan the whole path for the
	last char in the subpattern.  If not, the shared prefix
	doesn't match the pattern, so do not scan it for the last
	char.
****/
...
/****
	Search backward starting at the end of the path we just
	read in, for the character at the end of the last glob-free
	subpattern in PATHPART.
****/
...
/****
	Success on the fast match.  Compare the whole pattern if
	it contains globbing characters.
****/
...
static void usage (FILE *stream,int status) noex {
	fprintf (stream, "\
...
local int isfile(cchar *name) noex {
	ustat	sb ;
	int	rs1 ;
	int	namelen ;
	...
	rp = strwcpy(fname,name,MAXPATHLEN) ;

	while ((rp > fname) && ((*rp == '\r') || (*rp == '\n')) {
		rp -= 1 ;
		}

	rs1 = u_lstat(fname,&sb) ;
