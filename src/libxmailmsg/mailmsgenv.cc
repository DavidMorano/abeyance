...
/******************************************************************************
	...
	Description:
	This is a small collection of subroutines that form a simple
	object for parsing and storing the elements of an UNIX®
	message envelope.  There are many (too many) subroutines
	out there that detect, parse, or otherwise process UNIX®
	message envelope information.  I really wish that another
	set of subroutines was not needed.  But, no, we need to do
	just that simple little thing that nobody else currently
	does!  Is not this the way?!
...
#if	CF_GETTIME
extern time_t	getabsdate(cchar *,TIMEB *) ;
#endif
...
#if	CF_GETTIME
local int	mailmsgenv_proctime(MAILMSGENV *) ;
#endif /* CF_GETTIME */

local int	freeit(cchar **) noex ;
...
int mailmsgenv_start(MAILMSGENV *mep,cchar *sbuf,int slen) noex {
	int		rs = SR_OK ;
	int		i, ll ;
	int		ch ;
	...
	int		f_len, f_esc ;
	cchar	*cp ;
	cchar	*p_address = nullptr ;
	cchar	*p_date = nullptr ;
	cchar	*p_remote = nullptr ;
	cchar	*pd_day, *pd_month ;
	cchar	*cp2 ;
	cchar	*ccp ;
...
/* the next field should have one or more colons in it (the rest digits) */
	...
	ch = (cp2[0] & 0xff) ;
	if ((! isdigitlatin(ch)) || (strchr(cp2,':') == nullptr)) {
	    rs = SR_INVALID ;
	    goto bad0 ;
	}
...
/* handle bad things */
bad2:
	if (mep->remote != nullptr) {
	    uc_free(mep->remote) ;
	    mep->remote = nullptr ;
	}

bad1:
	if (mep->address != nullptr) {
	    uc_free(mep->address) ;
	    mep->address = nullptr ;
	}
...
int mailmsgenv_isstart(MAILMSGENV *mep) noex {
	if (mep == nullptr) return SR_FAULT ;

	if (mep->address == nullptr) return SR_NOTOPEN ;

	return (mep->f.start) ;
}
/* end subroutine (mailmsgenv_isstart) */

int mailmsgenv_getaddress(MAILMSGENV *mep,cchar **app) noex {
	if (mep == nullptr) return SR_FAULT ;
...
int mailmsgenv_getremote(MAILMSGENV *mep,cchar **app) noex {
	int		rs = SR_OK ;
...
int mailmsgenv_gettzname(MAILMSGENV *mep,cchar **app) noex {
...
int mailmsgenv_gettime(MAILMSGENV *mep,time_t *timep) noex {
	time_t		d ;
...
int mailmsgenv_mkdatestr(MAILMSGENV *mep,cc *name,char *buf,int buflen) noex {
	if (mep == nullptr) return SR_FAULT ;
...
int mailmsgenv_mkenv(MAILMSGENV *mep,char *rbuf,int rlen) noex {
	sbuf		ub ;
	int		rs ;
	int		rs1 ;
...
/* the date string */
...

	    {
	        ...
	        time_t	d = (mep->f.hastime) ? mep->daytime : time(nullptr) ;
...
/* the optional remote machine */
	    ...
	    if ((mep->remote != nullptr) && (mep->remote[0] != '\0')) {
...
int mailmsgenv_finish(MAILMSGENV *mep) noex {
	int		rs = SR_OK ;
	int		rs1 ;
...
local int mailmsgenv_proctime(MAILMSGENV *mep) noex {
	if (! mep->f.init_time) {
	    mep->f.init_time = true ;
	    if (mep->origdate != nullptr) {
	        time_t	d ;
	        if ((d = getabsdate(mep->origdate,nullptr)) >= 0) {
	            mep->f.hastime = true ;
	            mep->daytime = d ;
	        }
	    } /* end if */
...
local int freeit(cchar **pp) noex {
	int		rs = SR_OK ;
	if (*pp != nullptr) {
	    rs = uc_free(*pp) ;
	    *pp = nullptr ;
