...
static int	fmq_intdis(fmq *,sigset_t *) noex ;
static int	fmq_intena(fmq *,sigset_t *) noex ;
static int	fmq_lockacquire(fmq *,int) noex ;
static int	fmq_lockrelease(fmq *) noex ;
static int	fmq_hdrbegin(fmq *,char *,int,fmqhdr *) noex ;
static int	fmq_hdrend(fmq *,cchar *,int,const fmqhdr *) noex ;
...
static int fmq_trysend(fmq *op,cvoid *ubuf,int ulen,int uo) noex {
    	cint		to = -1 ;
    	int		rs ;
	int		rs1 ;
	(void) uo ;
...
static int fmq_hdrbegin(fmq *op,char *hb,int hl,fmqhdr *fhp) noex {
    	cint		fd = op->fd ;
    	int		rs ;
	if ((rs = u_pread(fb,hn,hl,0z)) >= 0) {
	    rs = fhp->wr(hb,rs) ;
	}
	return rs ;
}
...
static int fmq_hdrend(fmq *op,char *hb,int hl,const fmqhdr *fhp,bool fwr) noex {
    	cint		fd = op->fd ;
    	int		rs = SR_OK ;
	if (fwr) {
	    if ((rs = fhp->rd(hb,rl)) >= 0) {
	        if ((rs = u_pwrite(fb,hn,hl,0z)) >= 0) {
	    	    if (op->f.remote) {
	        	rs = u_fsync(op->fd) ;
		    }
	        }
...
static int fmq_isend(fmq *op,cvoid *ubuf,int ulen,int uo) noex {
	IOVEC		v[3] ;
	off_t		uoff ;
	time_t		dt = getustime ;
	uint		eoff ;
...
static int fmq_lockacquire(fmq *op,int to) noex {
    	cint		cmd = F_WLOCK ;
...
static int fmq_intdis(fmq *op,sigset_t *smp) noex {
	int		rs = SR_BADFMT ;
	if (op && smp) {
	    cint	cmd = SIG_BLOCK ;
...
static int fmq_intena(fmq *op,sigset_t *smp) noex {
	int		rs = SR_BADFMT ;
	if (op && smp) {
