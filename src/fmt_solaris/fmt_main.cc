...
local int csplit() noex ;
local int msplit() noex ;

local void fill_hdrbuf(wchar_t *line) noex ;
local void header_chk(void) noex ;
local void process_hdrbuf(void) noex ;
...
local void prefix(wchar_t *line) noex {
	wchar_t *cp;
	int np;
	int i;
...
local void csplit(wchar_t *line) noex {
	wchar_t *cp, *cp2;
	wchar_t word[BUFSIZ];
	...
	while (*cp) {
		...
		if (*cp == L'\0') {
			*cp2++ = L' ';
			if (wschr(srchlist, cp[-1]) != nullptr)
				*cp2++ = L' ';
		}
		while (iswspace(*cp)) {
			*cp2++ = *cp++;
...
local void msplit(wchar_t *line) noex {
	wchar_t *cp, *cp2, prev;
	wchar_t word[BUFSIZ];
	static const wchar_t *srchlist = (const wchar_t *) L".:!?";
...
local void pack(wchar_t *word) noex {
	wchar_t *cp;
	int s, t;
...
local void tabulate(wchar_t *line) noex {
	wchar_t *cp, *cp2;
	int b, t;
...
local void leadin() noex {
	...
	for (b = 0, cp = outbuf; b < l; b++)
		*cp++ = L' ';
	outp = cp;
} /* end subroutine (leadin) */

/*
 * Is s1 a prefix of s2??
 */

local void ispref(wchar_t *s1, wchar_t *s2) noex {
	while (*s1 != L'\0' && *s2 != L'\0') {
		if (*s1++ != *s2++)
			return (0);
	}
	return (1);
} /* end subroutine (ispref) */

/*
 * Set an input option
 */

local void setopt(char *cp) noex {
	static int ws = 0;
...
local void	*dlhandle = nullptr;

local void _wckind_init() noex {
	char	*locale;
	...
	if ((dlhandle = dlopen(path, RTLD_LAZY)) != nullptr) {
...
} /* end subroutine (_wckind_init) */

int _wckind(wchar_t wc) noex {
	return (*__wckind) (wc);
} /* end subroutine (_wckind) */

local int _wckind_c_locale(wchar_t wc) noex {
	int	ret;
	...
	return ret ;
} /* end subroutine (_wckind_c_locale) */
