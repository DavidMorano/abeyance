...
#define CF_REJECT	0	/* will reject messages without Subjects
			   	(CF_OBJECT must be defined also) */
#define UNBUFFERED	0	/* use unbuffered output */
...
constexpr cpcchar	defspooldirs[] = {
	SPOOLDIR1,
	SPOOLDIR2,
	nullptr
} ;
...
int main(int argc,mainv argv,mainv envv) struct proginfo	g ;
	...
	cchar	*pr = nullptr ;
	cchar	*spooldir = nullptr ;
	cchar	*fromnode = nullptr ;
	...
	if ((efname == nullptr) || (efname[0] == '\0')) efname = BFILE_STDERR ;
...
/* get the current time-of-day */
	...
	g.daytime = time(nullptr) ;

/* make some log entries */

	g.f.log = FALSE ;
	buf[0] = '\0' ;
	if ((g.logfname != nullptr) && 
...
/* check if the spool directory is on-line */
	...
	if (spooldir == nullptr) spooldir = getenv("MSGSDIR") ;
	if (spooldir == nullptr) spooldir = getenv("MSGS_SPOOLDIR") ;

	if (spooldir == nullptr) {
	    int	dmode = (R_OK|X_OK) ;
	    for (i = 0 ; defspooldirs[i] != nullptr ; i += 1) {
...
/* OK, now we try to actually read it! */
	...
	g.f.bounds = FALSE ;
	if ((len = breadln(bfp,lbuf,LINEBUFLEN)) > 0) {
...
/* "Send" mode - place messages in the spool area */
	...
	if (f_send) {
	    ...
	    } else if ((u.mailname != nullptr) && (u.mailname[0] != '\0'))
	        fromname = u.mailname ;
	    ...
	    if (! g.f.pcsconf) {
	        ...
	        g.f.pcsconf = TRUE ;
	        rs = pcsconf(g.pr,nullptr,&p, nullptr,nullptr,pcsbuf,PCSBUFLEN) ;
...
/* what about the user's MSGSRC file? */
	...
	if ((cp = getenv("MSGSRC")) == nullptr) {
	    cp = MSGSRC ;
	}

	rs = mkufname(ufname,u.homedname,cp) ;
	...
	if ((rs = bopen(ufp,ufname, "rw",0666)) >= 0) {
	    ...
	    if ((rs = bcontrol(ufp,BC_LOCK,LOCKTIMEOUT)) >= 0) {
	        ...
	        if ((l = breadln(ufp,lbuf,LINEBUFLEN)) > 0) {
...
/* * main program loop */
	...
	for (msg = firstmsg ; msg <= lastmsg ; msg += 1) {
	    ...
	    if ((newmsg = fopen(fname, "r")) == nullptr) continue ;
...
again:
	    ...
	    if ((hv_from != nullptr) && (hv_from[0] != '\0')) {
	    ...
	    if ((hv_date != nullptr) && (hv_date[0] != '\0')) {
	    ...
	    if ((hv_subj != nullptr) && (hv_subj[0] != '\0')) {
...
#ifndef NOMETAMAIL
	if ((getenv("NOMETAMAIL") == nullptr) && f_nontext) {
...
/* * handle 'mail' and 'save' here */
	...
	if ((c_inch == 's') || (c_inch == 'm')) {
	    ...
	    if (bopen(hfp,helpfname,"r",0666) >= 0) {
	        ...
	        while ((len = breadln(hfp,helpfname,MAXPATHLEN)) > 0) {
...
/* check if this user is in the MSGS cleanup group */
	...
	for (i = 0 ; glist[i] != nullptr ; i += 1) {
	    if ((gp = getgrnam(glist[j])) == nullptr) continue ;
...
/* check if this user is on our list of OK users */
	...
	i = 0 ;
	un = ulist[i] ;
	while ((un != nullptr) && (*un != '\0')) {

	    if (((pp = getpwnam(un)) != nullptr) &&
