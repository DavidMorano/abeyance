...
char *argv[] ;
{
	      ...
	      case PLUS:
		    ...
		    goto date;
	         ...
	         if (imat > 1)
	         {
	            ...
	            fprintf(stderr,
			    "%s: ambiguous option: %c%s (matches",
			    	argv[0],c_p.spec,c_p.opt);
	            for (j=0;j < imat;j++)
	      ...
	      if (strcmp(argv[k],"to") == 0 ||
	         ...
	         if((guess_yr[idate] = get_date(s,dates[idate])) < 0) {
	            ...
	            if (tok == NULL || (i = num_month(tok)) == 0) {
	            	fprintf(stderr,
			    "\"%s\" is an unknown date or calendar\n",ss);
	            	exit(1);
	            }
	            tok = strtok(0,"	 ");
	            if (tok == NULL) {
	...
	if (entries == 0) {
		if (speflag != 2) {
		    printf ("no entries for ");
		} else {
		    printf(
			"\n no entries on Computation Center Downtime "
			"Schedule for ");
	        }
