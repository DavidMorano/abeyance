...
char	mfname[] ;
{
	...
	char	buf[BUFLEN + 1] ;
	char	cmdbuf[(3 * MAXPATHLEN) + 50] ;


/* check for $HOME/mail/copy file */

	sprintf(buf,"%s/mail/copy", u.homedname) ;

#ifdef	COMMENT
	if ((rs = bopen(cfp,buf,"wca",0666)) >= 0) {

	    if ((rs = bopen(mfp,mfname,"r",0666)) >= 0) {

	        bprintf(cfp,"From %s %s\n",u.username,g.date_envelope) ;
	...
	if (rs < 0) {
	    ...
	    sprintf(buf,"rmail %s < %s",u.username,mfname) ;
...
#else
	sprintf(cmdbuf,"%s %s %s -e",g.prog_pcscl,
		mfname,buf) ;
