...
#include	"who_config.h"
#include	"who_defs.h"


/* local defines */

#define	INITTABFNAME	"/etc/inittab"
...
extern int	opterr ;	/* disable default error message */
...
local void	process(int) noex ;
local void	ck_file(cchar *) noex ;
local void	dump(void) noex ;

static UTMPX	*utmpp;	/* pointer for getutxent()	*/
...
static char	errmsg[BUFSIZ];	/* used in sprintf for errors	*/
static int	fildes = -1 ;		/* file descriptor for inittab	*/
...
static cchar	*program;	/* holds name of this program	*/
#ifdef	XPG4
...
static int	validtype[UTMPXTYPE+1];	/* holds valid types	*/
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	opterr = 0 ;
	gav = (char *const *) argv ;
	while ((optsw = getopt(argc,gav,"VabdHlmn:pqrstTu")) != EOF) {
...
/* check is '/etc/inittab' is readable */

#ifdef	DAM

		if (perm(INITTABFNAME,-1,-1,NULL,R_OK) >= 0)
			fildes = 0 ;
	...
	if (fildes >= 0) {
		...
		if ((inittab = malloc(stbufp->st_size + 1)) == NULL) {
			(void) sprintf(errmsg, gettext(
			    "%s: Cannot allocate %llu bytes"), program,
			    stbufp->st_size);
			perror(errmsg);
		...
		if ((fildes = open("/etc/inittab", O_RDONLY)) == -1) {
			(void) sprintf(errmsg,
			    gettext("%s: Cannot open /etc/inittab"),
			    program);
			perror(errmsg);
...
local void dump() noex {
	 ...
	 */
	lptr = &utmpp->ut_xtime;
	(void) cftime(time_buf, dcgettext(NULL, DATE_FMT, LC_TIME), lptr);
	...
	if (utmpp->ut_type == RUN_LVL) {
	    cint pid = int(utmpp->ut_pid) ;
		(void) printf("     %c  %5d  %c", pterm,pid,pexit) ;
	...
	if (utmpp && utmpp->ut_host[0]) {
	    cint hsz = szof(utmpp->ut_host) ;
		(void) printf("\t(%.*s)",hsz, utmpp->ut_host) ;
	}
...
local void process(int f_all) noex {
	PASSWD		*pwp;
	int i = 0;
	int	f ;
...
local void ck_file(cchar *name) noex {
	struct	stat sbuf;
	int	rc;
