...
#ident	"@(#)nlps_serv.c	1.8	98/10/22 SMI" /* SVr4.0 1.3.4.1	*/


/* this file has been modified by David A­D­ Morano (DAM) */

#define	CF_DEBUGS	0		/* compile-time debugging */
#define	CF_FORK		1		/* do a 'fork(2)' */
...
static int nlps_server(const char *) ;
...
int nlps_server(const char *provider)
{
	register size ;
	char buf[RCVBUFSZ] ;
	char **argv ;
...
/* * else if message is for the MS-NET file server...  */
	    ...
	    if (dbp = getdbfentry(DBF_SMB_CODE)) {
	        if (dbp->dbf_flags & DBF_OFF) {
	            logmessage(
			"NLPS: SMB message, server disabled in data base") ;
	        } else {
	            argv = mkdbfargv(dbp) ;
...
/* * if message is NLPS protocol...  */
	...
	if ((!strncmp(bp,NLPSIDSTR,NLPSIDSZ)) &&
	    (*(bp + NLPSIDSZ) == NLPSSEPCHAR)) {
...
/* SMB: read in 28 more bytes to get count of paramter words */
...
/* pitch the first */
	            ...
	            ret = ioctl(0, I_PEEK, &peek) ;
	            if (ret == -1) {
	                sprintf(scratch, "NLPS: nullfix(): unable to PEEK second time, errno is %d",
	                    errno) ;
...
/* open pipe to pass fd through */
				...
				"doconfig failed on line %d of script %s", 
	                ...
	                if (ioctl(passfd, I_SENDFD, 0) < 0) {
	                    sprintf(scratch,
				"NLPS passfd failed: %s", dbp->dbf_cmd_line) ;
	                    logmessage(scratch) ;
	                }
	            }
	        }
...
int nls_chkmsg(bp, size, lowp, highp, svc_code_p)
char *bp, *svc_code_p ;
int size, *lowp, *highp ;
{
...
Update note here: Someone above (not me) realized that the "reply"
should be sent *after* any optional modules have been pushed.  But the
original code had all replies being sent back *before* any modules
were pushed.  This is what caused a race condition on fast client
sending data before the intended modules for a server had been pushed.
I changed the code here (above) to always push any specified modules
before sending the NLPS reply.  Even though someone thought to only
allow |write(2)|s in this reply code, it was never done.  The original
reply code (below) had a |t_snd(3tli)| call instead of a |weite(2)|
call.  I changed it to use a |weite(2)| as is indeed properly needed.
-- David A­D­ Morano (DAM)

****/

static char *srrpprot = "%d:%d:%s" ;

int nls_reply(code,text)
int		code ;
const char	text[] ;
{
	int	rs = SR_OK ;
...
#ifdef	COMMENT /* changed -- DAM */
	    t_snd(0, scratch, strlen(scratch)+1, 0) ;
#else
	    rs = u_write(0,scratch,(strlen(scratch)+1)) ;
#endif /* COMMENT */
	}
	return rs ; /* there actually is not used return value */
}
/* end subroutine (nls_reply) */
...
/* log the service-code */

	snprintf(msgbuf,MAXPATHLEN,"NLPS: service=%s",
	    (dbp)?dbp->dbf_svc_code:DBF_SMB_CODE) ;
	logmessage(msgbuf) ;
...
*/
	        ...
	        if (minor(sbuf.st_rdev) < 100) {
	            sprintf(device, "%.9s%02.02d", 
			prefix, minor(sbuf.st_rdev)) ;
	            } else
	            sprintf(device, "%.8s%03.03d", 
			prefix, minor(sbuf.st_rdev)) ;
...
/* after pushmod, tli calls are questionable?		*/
	...
	if (dbp) {
	    rs = pushmod(NETFD, dbp->dbf_modules) ;
	    if (rs < 0) {
	        snprintf(msgbuf,MAXPATHLEN,
...
/* final log entry and send the NLPS response to the network initiator */

	sprintf(msgbuf,"NLPS (%s) exec: %s", 
	    (dbp)?dbp->dbf_svc_code:DBF_SMB_CODE, path) ;
	rs = nls_reply(NLSSTART, msgbuf) ;
#if	CF_DEBUGS
	nprintf(NDEBUGFNAME,"exec_cmd: nls_reply() rs=%d\n",rs) ;
#endif
	logmessage(msgbuf) ;
	...
	{
	    char *const *exenvv = (char *const *) environ ;
	    execve(path, argvp, exenvv) ;
	}
...
register char *svc_code_p ;
{
	...
	while (rd_dbf_line(dbfp, Dbf_line_buf, &Dbf_ent.dbf_svc_code, 
	    &Dbf_ent.dbf_flags, &Dbf_ent.dbf_id, &Dbf_ent.dbf_res1,
	    &Dbf_ent.dbf_res2, &Dbf_ent.dbf_res3,&Dbf_ent.dbf_prv_adr,
	    &Dbf_ent.dbf_prognum, &Dbf_ent.dbf_version, 
	    &Dbf_ent.dbf_modules, &Dbf_ent.dbf_sflags, 
	    &Dbf_ent.dbf_cmd_line) > 0) {

/* see if this line is the one we want (svc_code match) */
	    if (!strcmp(Dbf_ent.dbf_svc_code, svc_code_p)) {
	        fclose(dbfp) ;
	        return(&Dbf_ent) ;
	    }
	}
