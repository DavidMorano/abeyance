...
#ident	"@(#)lsdbf.c	1.3	96/11/26 SMI"	/* SVr4.0 1.8.5.1	*/
...
*
* New-note: No it does not!  I changed it to not write into its own
* arguments. -- DAM
*
****/
...
#if	CF_DEBUGS
	nprintf(NDEBUGFNAME,"pushmod: entered fd=%d\n",fd) ;
	nprintf(NDEBUGFNAME,"pushmod: mods=>%s<\n",mods) ;
#endif
...
#if	CF_DEBUGS
	nprintf(NDEBUGFNAME,"pushmod: ret rs=%d\n",rs) ;
#endif
