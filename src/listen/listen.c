...
#ident	"@(#)listen.c	1.10	98/10/22 SMI"	/* SVr4.0 1.19.9.1	*/
...
const char	*Progname;	/* listener's basename (from argv[0]) */
static	char Provbuf[PATHSIZE];
char	*Provider = Provbuf;	/* name of transport provider		*/
const char	*Netspec = NETSPEC;
const char	*Minor_prefix;	/* prefix for minor device names	*/
...
const char	*envv[] ;
{
...

	{
	    ...
	    while ((c = getopt(argc,gargv,"m:")) != EOF) {
		switch (c) {
		case 'm':
			Minor_prefix = optarg;
			break;
 ...
 */
	...
	Ndesc = ulimit(UL_GDESLIM,0L) ; /* get num of file des on system */
...
net_open()
{
	...
	/* Pending calls are linked in a structure, one per file descriptor */
	if ((Priv_call = (struct call_list *) 
		malloc(Ndesc *(sizeof( struct call_list)))) == NULL)  
...
register dbf_t *dbp;
{
	...
	if (dbp->dbf_flags & DBF_UTMP) {
		 ...
		 * the wtmpx file.
		 */
		snprintf(dummy,PMTAGSIZE,".%s", Mytag);
