...
/* create the values for the file schedule searching */
	...
	vecstr_envset(&schedvars,"p",g.programroot) ;

	vecstr_envset(&schedvars,"e","etc") ;

	vecstr_envset(&schedvars,"n",SEARCHNAME) ;
...
/* find a configuration file if we have one */
	...
	if ((configfname == nullptr) || (configfname[0] == '\0')) {
		...
		configfname = CONFIGFILE ;
	        ...
	        sl = permsched(sched1,&schedvars,
	            tmpfname,MAXPATHLEN, configfname,R_OK) ;
		if (sl < 0) {

	            sl = permsched(sched3,&schedvars,
	            tmpfname,MAXPATHLEN, configfname,R_OK) ;
		    if (sl > 0)
...
/* we must set this mode to 'VARSUBM_BADNOKEY' so that a miss is noticed */
	    ...
	    varsubinit(&vsh_d,VARSUBM_BADNOKEY) ;
...
/* program root from configuration file */
	    ...
	    if ((cf.root != nullptr) && (! f_programroot)) {
	        ...
	        if (((l = var_subbuf(&vsh_d,&vsh_e,cf.root,
	            ...
	            g.programroot = mallocsbuf(buf2,l2) ;
...
/* all of the rest of the configuration file stuff */
	    ...
	    if ((cf.user != nullptr) && (g.defuser == nullptr)) {
	        ...
	        if (((l = var_subbuf(&vsh_d,&vsh_e,cf.user,
	            -1,buf,BUFLEN)) > 0) &&
	            ((l2 = expander(&g,buf,l,buf2,BUFLEN)) > 0)) {

	            g.defuser = mallocsbuf(buf2,l2) ;
	    ...
	    if ((cf.group != nullptr) && (g.defgroup == nullptr)) {
	        ...
	        if (((l = var_subbuf(&vsh_d,&vsh_e,cf.group,
	            -1,buf,BUFLEN)) > 0) &&
	            ((l2 = expander(&g,buf,l,buf2,BUFLEN)) > 0)) {
	    ...
	    if ((cf.srvtab != nullptr) && (srvfname[0] == '\0')) {
	        ...
	        if (((l = var_subbuf(&vsh_d,&vsh_e,cf.srvtab,
	            -1,buf,BUFLEN)) > 0) &&
	            ((l2 = expander(&g,buf,l,buf2,BUFLEN)) > 0)) {
	    ...
	    if ((cf.sendmail != nullptr) && (g.prog_sendmail == nullptr)) {
	        ...
	        if (((l = var_subbuf(&vsh_d,&vsh_e,cf.sendmail,
	            -1,buf,BUFLEN)) > 0) &&
	            ((l2 = expander(&g,buf,l,buf2,BUFLEN)) > 0)) {
	            ...
	            g.prog_sendmail = mallocsbuf(buf2,l2) ;
...
/* what about an 'environ' file ? */
	    ...
	    if (cf.envfile != nullptr) {
	        ...
	        procfile(procfileenv,g.programroot,&schedvars,
	            ENVFILE,&exports) ;
...
/* "do" any 'paths' file before we process the environment variables */
	    ...
	    if (cf.pathsfile != nullptr)
	        ...
	        procfile(procfilepaths,g.programroot,&schedvars,
	            PATHSFILE,&exports) ;
...
/* do we have an activity log file ? */
	...
	rs = logfile_open(&g.lh,logfname,0,0666,buf) ;

	gp->logid = mallocstr(buf) ;
...
/* handle UID/GID stuff */
	...
	rs = getgr_gid(g.gid,&ge,buf,BUFLEN) ;
...
/* initialize a SRVPE (Server Process Entry) object for use later */

	srvpe_init(&spe,g.version,g.programroot,g.nodename,g.domainname) ;
...
/* find and open the server file */
	...
	if (srvfname[0] == '\0') {
	    ...
	    if ((sl = permsched(sched2,&schedvars,
...
/* we are done initializing */
	...
	if (g.f.daemon) {
	    ...
	    srs = watch(&g,&spe,s,&exports,&sfile,&bis) ;
	    ...
	    srs = watchone(&g,&spe,s,&exports,&sfile,&bis) ;
