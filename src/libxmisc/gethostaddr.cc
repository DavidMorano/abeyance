/* gethostaddr SUPPORT (UNFINISHED) */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* subroutine to get a host INET address */
/* version %I% last-modified %G% */


/* revision history:

	= 1998-11-01, David A­D­ Morano
	This program was originally written.

*/

/* Copyright © 1998 David A­D­ Morano.  All rights reserved. */

/*******************************************************************************

	Name:
	gethostaddr

	Synopsis:
	int gethostaddr(HE *hep,char *hebuf,int helen,cc *name) noex

*******************************************************************************/

#include	<envstandards.h>	/* ordered first to configure */
#include	<sys/types.h>
#include	<sys/param.h>
#include	<sys/socket.h>
#include	<netinet/in.h>
#include	<arpa/inet.h>
#include	<unistd.h>
#include	<fcntl.h>
#include	<netdb.h>
#include	<cstddef>		/* |nullptr_t| */
#include	<cstdlib>
#include	<cstring>
#include	<clanguage.h>
#include	<usysbase.h>
#include	<getxname.h>
#include	<localmisc.h>


/* local defines */

#ifndef	GETHOSTADDR_SYSV
#define	GETHOSTADDR_SYSV	SYSHAS_GETHOSTXXXR
#endif


/* external subroutines */


/* external variables */


/* local structures */


/* forward references */


/* local structures */


/* exported variables */


/* exported subroutines */

int gethostaddr(ucentho *hep,char *hebuf,helen,cc *name) noex {

	int	rs ;

#if	GETHOSTADDR_SYSV
	int	h_errno_local ;
#endif

	char	nnbuf[MAXHOSTNAMELEN+1] ;


	if (hep == nullptr)
	    return SR_FAULT ;

	if (hostbuf == nullptr)
	    return SR_FAULT ;

	if (name == nullptr) {
	    name = nnbuf ;
	    rs = getnodename(nnbuf,MAXHOSTNAMELEN) ;
	}
	if (rs >= 0) {
	    hostent	he, *lp ;

#if	GETHOSTADDR_SYSV
	h_errno_local = 0 ;
	do {
	    lp = gethostbyname_r(name,
	        &he,hostbuf,hostlen,&h_errno_local) ;
	    if ((lp == nullptr) && (h_errno_local == netdberr.tryagain)) {
		sleep(1) ;
	    }
	} while ((lp == nullptr) && (h_errno_local == netdberr.tryagain)) ;
#else
	do {
	    lp = gethostbyname(name) ;
	    if ((lp == nullptr) && (h_errno == netdberr.tryagain)) {
		sleep(1) ;
	    }
	} while ((lp == nullptr) && (h_errno == netdberr.tryagain)) ;
#endif /* GETHOSTADDR_SYSV */

	if (lp) {
	    *hep = *lp ;
	} else {
	    rs = SR_NOTFOUND ;
	}

	} /* end if (ok) */

	return rs ;
}
/* end subroutine (gethostaddr) */


