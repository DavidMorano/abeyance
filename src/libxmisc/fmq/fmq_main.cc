...
local int fmq_ctor(fmq *op,Args ... args) noex {
    	FMQ		*hop = op ;
	int		rs = SR_FAULT ;
	if (op && (args && ...)) {
	    rs = memclear(hop) ;
...
local int fmq_dtor(fmq *op) noex {
	int		rs = SR_FAULT ;
	if (op) {
	    rs = SR_OK ;
	} /* end if (non-null) */
...
local int	fmq_opener(fmq *,time_t) noex ;
local int	fmq_fnbegin(fmq *,cchar *) noex ;
local int	fmq_fnend(fmq *) noex ;

local int	fmq_intdis(fmq *,sigset_t *) noex ;
local int	fmq_intena(fmq *,sigset_t *) noex ;
local int	fmq_lockacquire(fmq *,int) noex ;
local int	fmq_lockrelease(fmq *) noex ;
local int	fmq_hdrbegin(fmq *,char *,int,fmqhdr *) noex ;
local int	fmq_hdrend(fmq *,cchar *,int,const fmqhdr *) noex ;

local int	fmq_isend(fmq *,cvoid *,int,int) noex ;
local int	fmq_irecv(fmq *,void *,int,int) noex ;
local int	fmq_filecheck(fmq *,time_t,int,int) noex ;
local int	fmq_filebegin(fmq *,time_t) noex ;
local int	fmq_filechanged(fmq *) noex ;
local int	fmq_fileready(fmq *,time_t) noex ;
local int	fmq_filecreate(fmq *) noex ;
local int	fmq_fileopen(fmq *,time_t) noex ;
local int	fmq_fileclose(fmq *) noex ;
local int	fmq_bufbegin(fmq *) noex ;
local int	fmq_bufend(fmq *) noex ;
local int	fmq_headwrite(fmq *) noex ;
local int	fmq_intdis(fmq *,sigset_t *) noex ;
local int	fmq_intena(fmq *,sigset_t *) noex ;

#ifdef	COMMENT
local int	fmq_bufadj(fmq *) noex ;
#endif

#ifdef	COMMENT
local int	fmq_headread(fmq *) noex ;
#endif

local int	filemagic(char *,int,fmq_fm *) noex ;

local int filehead_wr(fmq_fh *,cchar *) noex ;
local int filehead_rd(const fmq_fh *,char *) noex ;


/* local variables */

constexpr int	fmqhdr_bufsize = (szof(fmqhdr) + FMQHDR_MAGICSIZE) ;
...
local int fmq_fnbegin(fmq *op,cchar *fname) noex {
    	int		rs = SR_OK ;
	int		rs1 ;
	cchar		*fnp{} ;
...
local int fmq_fnend(fmq *op) noex {
    	int		rs = SR_OK ;
	int		rs1 ;
	if (op->fname) {
...
local int fmq_opener(fmq *op,time_t dt) noex {
	int		rs ;
	op->tiopen = dt ;
	op->tiaccess = dt ;
...
local int fmq_trysend(fmq *op,cvoid *ubuf,int ulen,int uo) noex {
    	cint		to = -1 ;
    	int		rs ;
	int		rs1 ;
	(void) uo ;
    	if (sigset_t sm{} ; (rs = fmq_intdis(op,&sm)) >= 0) {
	    if ((rs = fmq_lockacquire(op,to)) >= 0) {
		cint	hsz = fmqhdr_bufsize ;
		if (char *hb{} ; (rs = uc_malloc(hsz,&hb)) >= 0) {
		    cint	hl = rs ;
		    if (fmqhdr fh{} ; (rs = fmq_hdrbegin(op,hb,hl,&fh)) >= 0) {
		        bool	fwr = false ;
	                {
        
		        }
		        rs1 = fmq_hdrend(op,hb,hl,&fb,fwr) ;
			if (rs >= 0) rs = rs1 ;
	            } /* end if (fmq_hdr-) */
		    rs = rsfree(rs,hb) ;
		} /* end if (m-a-f) */
	        rs1 = fqm_lockrelease(op) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (file-lock) */
	    rs1 = fmq_intena(op,&sm) ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (fmq-block-interrupts) */
	return rs ;
} /* end subroutine (fmq_trysend) */

local int fmq_hdrbegin(fmq *op,char *hb,int hl,fmqhdr *fhp) noex {
    	cint		fd = op->fd ;
    	int		rs ;
	if ((rs = u_readp(fb,hn,hl,0z)) >= 0) {
	    rs = fhp->wr(hb,rs) ;
	}
	return rs ;
} /* end subroutine (fmq_hdrbegin) */

local int fmq_hdrend(fmq *op,char *hb,int hl,const fmqhdr *fhp,bool fwr) noex {
    	cint		fd = op->fd ;
    	int		rs = SR_OK ;
	if (fwr) {
	    if ((rs = fhp->rd(hb,rl)) >= 0) {
	        if ((rs = u_writep(fb,hn,hl,0z)) >= 0) {
	    	    if (op->f.remote) {
	        	rs = u_fsync(op->fd) ;
		    }
	        }
	...
	return rs ;
} /* end subroutine (fmq_hdrend) */

local int fmq_isend(fmq *op,cvoid *ubuf,int ulen,int uo) noex {
	IOVEC		v[3] ;
	off_t		uoff ;
	time_t		dt = getustime ;
	uint		eoff ;
...
#ifdef	COMMENT
local int fmq_isend(fmq *op,cvoid *buf,int buflen,int opts) noex {
	IOVEC		v[3] ;
	sigset_t	oldsigmask ;
	off_t		uoff ;
	time_t		dt = getustime ;
...
local int fmq_irecv(fmq *op,void *buf,int buflen,int opts) noex {
	IOVEC		v[3] ;
	USTAT		sb ;
	sigset_t	oldsigmask ;
	off_t		uoff ;
...
local int fmq_filecheck(fmq *op,time_t dt,int f_read,int opts) noex {
	int		rs ;
	int		f_changed = false ;
	(void) opts ;
	if (dt == 0) dt = getustime ;
...
local int fmq_filechanged(fmq *op) noex {
	int		rs ;
	int		fch = false ;
	if (USTAT sb ; (rs = u_fstat(op->fd,&sb)) >= 0) {
...
local int fmq_bufbegin(fmq *op) noex {
        cint		bsz = op->bufsize ;
	int		rs = SR_OK ;
	if (op->b.buf == nullptr) {
	    op->f.bufvalid = false ;
...
local int fmq_bufend(fmq *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (op->b.buf) {
...
local int fmq_filebegin(fmq *op,time_t dt) noex {
	fmq_fm		fm ;
	sigset_t	oldsigmask ;
	int		rs = SR_OK ;
	int		bl ;
...
/* write the file header stuff */
	    ...
	    rs = u_readp(op->fd,fbuf,FBUFLEN,0z) ;
...
local int fmq_lockacquire(fmq *op,int to) noex {
    	cint		cmd = F_WLOCK ;
    	return uc_lockfile(op->fd,cmd,0z,0z,to) ;
} /* end subroutine (fmq_lockacquire) */

local int fmq_lockrelease(fmq *op) noex {
    	cint		cmd = F_ULOCK ;
	cint		to = 0 ; /* unused */
    	return uc_lockfile(op->fd,cmd,0z,0z,to) ;
} /* end subroutine (fmq_lockrelease) */

local int fmq_fileready(fmq *op,time_t dt) noex {
	int		rs = SR_OK ;
	if (op->fd < 0) {
	    if (dt == 0) dt = getustime ;
...
local int fmq_fileopen(fmq *op,time_t dt) noex {
	int		rs = SR_OK ;
	if (op->fd < 0) {
...
local int fmq_filecreate(fmq *op) noex {
...
local int fmq_fileclose(fmq *op) noex {
	int		rs = SR_OK ;
	int		rs1 ;
	if (op->fd >= 0) {
	    rs1 = u_close(op->fd) ;
...
local int fmq_headwrite(fmq *op) noex {
	coff		uoff = FMQ_HEADOFF ;
	int		rs ;
	char		fbuf[FBUFLEN + 1] ;
	if ((rs = filehead_rd(&op->h,fbuf)) >= 0) {
	    rs = u_writep(op->fd,fbuf,rs,uoff) ;
	}
	return rs ;
} /* end subroutine (fmq_headwrite) */

local int fmq_intdis(fmq *op,sigset_t *smp) noex {
	int		rs = SR_BADFMT ;
	if (op && smp) {
	    cint	cmd = SIG_BLOCK ;
	    rs = u_sigmask(cmd,&op->sigmask,smp) ;
	} /* end if (non-null) */
	return rs ;
} /* end subroutine (fmq_intdis) */

local int fmq_intena(fmq *op,sigset_t *smp) noex {
	int		rs = SR_BADFMT ;
	if (op && smp) {
	    cint	cmd = SIG_SETMASK ;
	    rs = u_sigmask(cmd,smp,nullptr) ;
	} /* end if (non-null) */
	return rs ;
} /* end subroutine (fmq_intena) */

#ifdef	COMMENT
local int fmq_bufadj(fmq *op) noex {
    	int		rs = SR_OK ;
...
local int filemagic(char *buf,int f_read,fmq_fm *mp) noex {
	int		rs = SR_BADFMT ;
	if (buf && mp) {
	    char	*bp = buf ;
	    rs = 20 ;
...
local int filehead_wr(fmq_fh *hp,cchar *buf) noex {
...
local int filehead_rd(const fmq_fh *hp,char *buf) noex {
	uint		*table = (uint *) buf ;
	int		rs = SR_BADFMT ;
	if (buf) {
