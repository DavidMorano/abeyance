...
#define	CF_LOGIN	0		/* 'exec(2)' 'login(1)' ? */
#define	CF_SECURITY	1		/* enforce security */
#define	CF_EXEC		1		/* just 'execv(2)' */
...
#if	defined(P_TELNET) && (P_TELNET > 0)
#define	PROGNAME	"telnetd"
#endif

#if	defined(P_TELSERV) && (P_TELSERV > 0)
...
/* OK, continue with other stuff */
	...
	if (terminaltype != NULL) {
		...
		f_secure = termsecure((terminaltype + 5),svc) ;

#if	CF_DEBUGS
		syslog(LOG_DEBUG, "term_secure=%u\n",f_secure) ;

		syslog(LOG_DEBUG, "term=%s svc=%s\n",
			(terminaltype + 5),svc) ;
#endif
...
#if	CF_DEBUGS
		nprintf(DFNAME,"doit: write attempt domain=%s\n",cp) ;
#endif

		if ((cp != NULL) && (matstr(domains,cp,-1) >= 0)) {

			f_secure = TRUE ;
			f_write = TRUE ;
			strcpy((terminaltype + 5),"ansi_write") ;

			syslog(LOG_NOTICE, "write attempt");
...
/* System V login expects a UTMP entry to already be there */
	...
	if (f_write) {
		(void) strncpy(ut.ut_user,USERNAME_WRITE, sizeof(ut.ut_user));
	...
	if (f_write) {
		...
		(void) strncpy(ut.ut_user,USERNAME_GUEST, sizeof(ut.ut_user));

#endif /* CF_LOGIN */

		(void) strncpy(ut.ut_line, (line + 5), sizeof(ut.ut_line));
...
#if	CF_DEBUGS
		nprintf(DFNAME,"doit: mkutmpid()\n") ;
		nprintf(DFNAME,"doit: termdev=%s\n",line) ;
#endif

		rs = mkutmpid(ut.ut_id,4,line,-1) ;

#if	CF_DEBUGS
		nprintf(DFNAME,"doit: mkutmpid() rs=%d id=>%w<\n",
...
/* clear the supplemtal group list */
	...
	execl("/bin/login", "login", "-p", "-d", slavename, 
		"-h", host,NULL); /* XXX */
		...
		username = (f_write) ? USERNAME_WRITE : USERNAME_GUEST ;

		rs = getpw_name(&pe,pwbuf,PWBUFLEN,username) ;

		if (rs < 0)
			u_exit(EX_NOUSER) ;

		u_chown(slavename,pe.pw_uid,-1) ;
...
/* make a PAM session since nobody else is doing it for us! */
	...
	cp = "telnet" ;
			...
			if ((pam_start(cp, username, NULL, &pamh))
							== PAM_SUCCESS) {
				(void) pam_set_item(pamh, PAM_TTY, slavename);
...
/* continue with the thing */

	u_setgid(pe.pw_gid) ;

	u_setuid(pe.pw_uid) ;

	syslog(LOG_INFO, "exec username=%s\n",username) ;

	args[0] = "-ksh" ;
	args[1] = NULL ;

	i = 0 ;
	envs[i++] = "SVCINITIATOR=telserv" ;

	envs[i++] = "PATH=/bin" ;

	sncpy2(buf,BUFLEN,"TELSERV_REMOTEHOST=",host) ;

	envs[i++] = mallocstr(buf) ;

	sncpy2(buf,BUFLEN,"LOGNAME=",username) ;

	envs[i++] = mallocstr(buf) ;

	sncpy2(buf,BUFLEN,"HOME=",pe.pw_dir) ;

	envs[i++] = mallocstr(buf) ;

	sncpy2(buf,BUFLEN,"PWD=",pe.pw_dir) ;

	envs[i++] = mallocstr(buf) ;

/* the front part of this has the 'TERM=' characters */
	envs[i++] = terminaltype ;

	envs[i] = NULL ;

	rs = u_chdir(pe.pw_dir) ;

	if (rs >= 0) {
	    ex = EX_NOEXEC ;
	    u_execve("/bin/ksh",args,envs) ;
	}
	...
	syslog(LOG_INFO, "su username=%s\n",username) ;

		execl("/bin/su", "su", "-",username,NULL) ;
#endif /* CF_EXEC */
...
cchar	service[] ;
{
	...
	while ((len = fgetline(fp,lbuf,LINEBUFLEN)) > 0) {
...
int	fd ;
{
	...
	if ((rs = uc_tcgetattr(fd,&ts)) >= 0) {
	    if (f_needpop) rs = uc_tcsetattr(fd,TCSANOW,&ts) ;
	}
